...
def abstracts_from_files(abstracts, authors):
...
def generate_tex(abstracts):
    ...
    template = latex_jinja_env.get_template('extended-abstracts.tex.tmpl')
    with open('extended-abstracts.tex', 'w') as f:
        f.write(template.render(abstracts=abstracts))


if __name__ == '__main__':
    if len(sys.argv) < 3:
        print('You must include the extended abstract and author data CSVs as the arguments to this function.')
        sys.exit(1)

    authors = get_authors(sys.argv[2])
    generate_tex(abstracts_from_files(sys.argv[1], authors))
